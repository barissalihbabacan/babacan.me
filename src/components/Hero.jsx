  ...
  return (
    <section id="hero" className="relative min-h-screen flex flex-col justify-center pt-16">
      <div className="max-w-container-max mx-auto px-margin-desktop w-full py-[8vh]">
        ...
        <div className="flex items-center gap-4 mb-[4vh]">
          ...
          <span className="font-label-mono text-[11px] text-primary uppercase tracking-widest">
            Software Engineer · Systems Architect · Narrative Designer
          </span>
        </div>
        ...
        <div className="border-t border-primary/30 pt-[4vh] grid grid-cols-2 md:grid-cols-4 gap-5 max-w-2xl">
          <div>
            <div className="text-2xl font-bold text-on-surface font-display-lg">{t('hero.stat1_val')}</div>
            <div className="font-label-mono text-[10px] text-on-surface-variant uppercase tracking-widest mt-1">{t('hero.stat1_lbl')}</div>
          </div>
          <div>
            <div className="text-2xl font-bold text-on-surface font-display-lg">{t('hero.stat2_val')}</div>
            <div className="font-label-mono text-[10px] text-on-surface-variant uppercase tracking-widest mt-1">{t('hero.stat2_lbl')}</div>
          </div>
          <div>
            <div className="text-2xl font-bold text-on-surface font-display-lg">{t('hero.stat3_val')}</div>
            <div className="font-label-mono text-[10px] text-on-surface-variant uppercase tracking-widest mt-1">{t('hero.stat3_lbl')}</div>
          </div>
          <div>
            <div className="text-2xl font-bold text-on-surface font-display-lg">{t('hero.stat4_val')}</div>
            <div className="font-label-mono text-[10px] text-on-surface-variant uppercase tracking-widest mt-1">{t('hero.stat4_lbl')}</div>
          </div>
        </div>
      </div>
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 text-on-surface/55">
        <span className="font-label-mono text-[9px] uppercase tracking-[0.2em]">{t('hero.scroll')}</span>
