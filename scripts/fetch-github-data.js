...
async function fetchGitHubData() {
  try {
    ...
    const reposRes = await fetch(`https://api.github.com/users/${GITHUB_USER}/repos?sort=stars&per_page=100`, { headers });
    ...
    const sinsRes = await fetch(`https://api.github.com/orgs/thesinsofthefathers/repos?sort=stars&per_page=30`, { headers });
    const sinsRepos = sinsRes.ok ? await sinsRes.json() : [];

    console.log('Fetching org repositories (Osmos-App)...');
    const osmosRes = await fetch(`https://api.github.com/orgs/Osmos-App/repos?sort=stars&per_page=30`, { headers });
